define(
  ...
  function ($, oj, ko, GeneralViewModel, RESTConfig, AjaxUtils) {
    var theKey = {};
    ...
    function GraphicViewModel(params) {
      ...
      self.exportFormId = `${self.graphicId}-export-form`;
      self.exportURL = RESTConfig.export.graphic.path;
      ...
      self.downloadClickHandler = function() {
        const mappedData = self.getMonthlySeries()
          .map(e => {
              const {id, elementType: type, name, items} = e;
        ...
        const reducedData = mappedData.reduce(
            (hash, e) => {
              ...
              hash[e.id].items = e.items.reduce(
                  (hash, item) => {
                    const date = item.x.substr(0, 10);
                
                    if (!hash[date]) {
                      hash[date] = {};
                    }
                    
                    hash[date][e.type] = item.y;
                    hash[date]['date'] = date;
                    return hash;
                  },
                  hash[e.id].items
        ...
        const data = Object.keys(reducedData).map(
            itemKey => {
              const { name, items } = reducedData[itemKey];
