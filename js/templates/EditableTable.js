    ...
    function (ko, oj, $, GeneralViewModel, EventTypes, IdGenerator) {
      ...
      function EditableTable(model, params) {
        ...
        let privateData = {
          ...
          actions: ["go", "filter", "delete", "clone", "read", "edit"],
        ...
        self.editHandler = function (event, data) {
          if (data.rowContext) {
            let rowIdx = data.rowContext.status.rowIndex;
        ...
        self.deleteHandler = function () {
          const currentRow = self.getCurrentRow();
      ...
      prototype.visitItem = function (id, key) {
        if (theKey === key) {
          const visitor = this.getItemVisitor();
          return visitor(id);
        }
      };
